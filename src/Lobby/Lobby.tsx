...
const Lobby = (props: any) => {
  ...
  if (
    !String(localStorage.getItem('creatorRoom')).includes(roomId) ||
    !String(localStorage.getItem('creatorRoom')).includes('true')
  ) {
  ...
  const checkGuess = (event: any) => {
    ...
    const titleWithoutAccent = track.title_short
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase();
    const artistWithoutAccent = track.artist.name
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase();
    const guessWithoutAccent = guess
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase();
  ...
  return (
    <>
      ...
      <Grid style={{ width: '250px', margin: '5px' }} container spacing={2}>
        <Grid item>{volume >= 50 ? <VolumeUp /> : <VolumeDown />}</Grid>
        <Grid item xs>
          <Slider
            value={volume}
        ...
        {!gameIsFinished ? (
          <>
            ...
            {!gameIsStarted ? (
              ...
              >
                <Button
                  disabled={!isCreatorRoom || String(isCreatorRoom) == 'null'}
                  onClick={startGame}
                  variant="contained"
                  color="primary"
                  size="large"
              ...
              <Grid container direction="column">
                ...
                >
                  <form onSubmit={checkGuess}>
                    <TextField
                      disabled={artistIsGuessed && titleIsGuessed}
                      value={guess}
                      style={{ width: '40em' }}
                      onChange={handleGuessChange}
                      label={
                        artistIsGuessed && titleIsGuessed
                          ? 'Bien joué'
                          : 'Titre/Artiste bg ?'
                      }
                      autoFocus
                  ...
                  {artistIsGuessed ? (
                    <Mic
                      style={
                        artistIsGuessed ? { color: 'red' } : { color: 'black' }
                      }
                    />
                  ) : null}
                  {titleIsGuessed ? (
                    <Album
                      style={
                        titleIsGuessed ? { color: 'green' } : { color: 'black' }
                      }
                    />
                  ) : null}
                </div>
